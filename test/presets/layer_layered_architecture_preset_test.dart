...
void main() {
  final preset = LayerLayeredPreset();

  group('LayerLayeredPreset — contract', () {
...
'''));
  ...
  group('LayerLayeredPreset — rule properties', () {
    ...
    test('severity can be overridden globally', () {
      final rules = preset.expand(_cfg('severity: warning\n$_threeLayerYaml'));
      for (final r in rules) {
  ...
  group('LayerLayeredPreset — predicate evaluation', () {
    ...
    test('exceptions are excluded via selector excludeNames', () {
      final rules =
          preset.expand(_cfg('exceptions: [LegacyGod]\n$_threeLayerYaml'));
      for (final r in rules) {
        expect(
            (r.selector as ClassSelector).excludeNames, contains('LegacyGod'));
      }
    });
  });
}
